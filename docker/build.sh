...
source $GITHUB_WORKSPACE/docker/defaults.env
...
BUILD_NGINX="docker build -f nginx/Dockerfile . -t bahmni-msf-primero/nginx:${tag} -t ${repository}/bahmni-msf-primero/nginx:${tag} --build-arg NGINX_UID=${NGINX_UID} --build-arg NGINX_GID=${NGINX_GID} --build-arg BUILD_REGISTRY=${build_registry}"
BUILD_SOLR="docker build -f solr/Dockerfile ../ -t bahmni-msf-primero/solr:${tag} -t ${repository}/bahmni-msf-primero/solr:${tag} --build-arg BUILD_REGISTRY=${build_registry}"
